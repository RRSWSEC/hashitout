...
def _substitution_score(text: str) -> float:
    ...
    score = 0.0
    for i in range(len(s)-3):
        score += _SUB_TETRAGRAMS.get(s[i:i+4], -4.5)
...
def _initial_sub_key(ciphertext: str):
    ...
    remaining = [c for c in _EN_FREQ if c not in ordered]
    plain_order = ordered + remaining
    key = {cipher: plain for cipher, plain in zip(plain_order, _EN_FREQ)}
...
def _likely_substitution_cipher(text: str) -> bool:
    ...
    if len(s) < 60:
        return False
    unique = len(set(s))
    ic = _index_of_coincidence(s) if ' _index_of_coincidence' in globals() else 0.0
...
def _artifact_triage_text(text: str = '', raw: bytes = b'') -> str:
    bits = []
    base = globals().get('_artifact_triage_text_v49_prev')
    if callable(base):
        try:
            prior = base(text, raw)
            if prior:
                bits.extend([b.strip() for b in prior.split(';') if b.strip()])
        except Exception:
            pass
    if text:
...
def _finalize_findings(findings, source_label, wordlist=None):
    ...
    for f in findings or []:
        ...
        f.score = _score_candidate(
            f.result_text or '', f.result_bytes or b'', wordlist, f.confidence
        )
    ...
    out.sort(key=lambda f: (conf_rank.get(f.confidence, 0), f.score, -f.entropy), reverse=True)
    return out
...
def _patched_text_quality(self, text: str):
    ...
    if score >= 52 or (ratio > 0.96 and alpha > 0.55 and (is_mostly_words(text, self.wordlist, 0.20) if self.wordlist else False)):
        return ('HIGH', f'scored {score} with strong plaintext indicators')
    if score >= 28 or (ratio > 0.92 and alpha > 0.40):
...
def _shell_show_finding(f, idx):
    _shell_show_finding_v50_prev(f, idx)
